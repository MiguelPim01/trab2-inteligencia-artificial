...
class NeuralNetworkAgent(Agent):
    ...
    """
    def __init__(self, weight_vector: np.ndarray, input_layer : int = 27, hidden1_layer : int = 32, hidden2_layer : int = 16, output_layer : int = 3):
        ...
        self._unpack_genes(weight_vector)

    def _unpack_genes(self, weight_vector: np.ndarray):
        """Separa os pesos do vetor pelas camadas da rede

        Args:
            weight_vector (np.ndarray): Vetor de pesos
        """
        idx = 0
        ...
        def extract_weights(in_dim, out_dim):
            ...
            w = weight_vector[idx : idx + w_size].reshape((in_dim, out_dim))
            idx += w_size
